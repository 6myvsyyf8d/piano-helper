...
function renderCalendar() {
  ...
  const contentHTML = calendarState.view === 'week'
    ? buildWeekViewHTML()
    : buildCalendarHTML(calendarState.year, calendarState.month);

  page.innerHTML = viewToggle + contentHTML;
  bindCalendarEvents();
...
function buildCalendarHTML(year, month) {
  ...
  for (let i = firstDay - 1; i >= 0; i--) {
    ...
    cells += `<div class="calendar-day other-month" data-date="${dateStr}">${d}</div>`;
  }
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    const starCount = log ? log.entries.reduce((s, e) => s + (e.rating || 0), 0) : 0;
    const starHtml = starCount > 0 ? `<span class="day-star">${starCount}⭐</span>` : '';

    cells += `<div class="${classes.join(' ')}" data-date="${dateStr}">${d}${starHtml}</div>`;
  }
...
function buildWeekViewHTML() {
  ...
  const dayHeaders = ['日', '一', '二', '三', '四', '五', '六'];
  const logsMap = {};
  DB.logs().forEach(log => { logsMap[log.date] = log; });
  ...
  for (let i = 0; i < 7; i++) {
    ...
    const duration = log ? (log.totalDurationMin || 0) : 0;
    const stars = log ? log.entries.reduce((s, e) => s + (e.rating || 0), 0) : 0;
  ...
  const totalDays = Object.keys(logsMap).filter(d => {
    const t = new Date(d + 'T00:00:00');
    return t >= weekStart && t <= weekEnd;
  }).length;
  const totalMin = Object.entries(logsMap).reduce((s, [d, l]) => {
    const t = new Date(d + 'T00:00:00');
    return s + (t >= weekStart && t <= weekEnd ? (l.totalDurationMin || 0) : 0);
  }, 0);
